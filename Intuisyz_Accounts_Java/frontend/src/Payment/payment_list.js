...
const Payment_list = () => {
  ...
  function deleting(tranID) {
    ...
    axios
      // .delete(`http://localhost:8080/payment_delete/${tranID}`)
  ...
  const submitFinal = handleSubmit((data) => {
    if (startDate === null || startDate === undefined) {
      ...
      axios
        // .get(`http://localhost:8080/payment_bn_date?start=${sDate}&end=${eDate}`)
        .get(
          url +
            `payment_bn_date?start=${sDate}&end=${eDate}&CompanyName=${sessionStorage.getItem(
              'CompanyName'
  ...
  function sorting(field, type) {
    ...
    axios
      // .get(
      //   `http://localhost:8080/transaction_sorting?field=${field}&type=${type}`
      // )
      .get(
        url +
          `transaction_sorting?field=${field}&type=${type}&CompanyName=${sessionStorage.getItem(
            'CompanyName'
          )}&CustId=${sessionStorage.getItem('CustId')}`
      )
      .then(({ data }) => {
  ...
  function dataLoading() {
   ...
   {/*     let k = data[0].tran_Date;
    ...
    axios
      // .get('http://localhost:8080/list_transaction')
      .get(
        url +
          `list_transaction?CompanyName=${sessionStorage.getItem(
            'CompanyName'
          )}&CustId=${sessionStorage.getItem('CustId')}`
      )
      .then(({ data }) => {
