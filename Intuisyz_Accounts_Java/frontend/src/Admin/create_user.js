...
const Create_user = () => {
  ...
  const submitFinal = handleSubmit((data) => {
    ...
    axios
      .get(url + `userSearch?userName=${user_name}`)
      .then(({ data }) => {
    ...
    axios
      ...
      .then(({ data }) => {
        ...
        if (data.length > 0) {
          ...
          console.log('data[0].cashId ',data[0].cashId,' data[0].gstId ',data[0].gstId,' data[0].tdsId ',data[0].tdsId);
          ...
          finalObj6.template_companyName = company_name;
