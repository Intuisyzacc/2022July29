...
public class LedgerService {
	...
	public List<Account_ledger_v3> ledger_name_searchs(String ledgerName,String CompanyName,String CustId)  {
		System.out.println(ledgerName);
		
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.ledger_name_search(ledgerName,CompanyName,CustId);
	...
	public List<Account_ledger_v3> list_ledgers(String CompanyName,String CustId)  {
		
		
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.selectData(CompanyName,CustId);
...
public List<Account_ledger_v3> ledger_sorts(String field, String type,String CompanyName,String CustId)  {
		...
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.ledger_nameA(CompanyName,CustId);
		}
		
		if(field.equals("ledger_name")&&type.equals("DESC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.ledger_nameD(CompanyName,CustId);
		}
		
		if(field.equals("ac_group")&&type.equals("ASC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.ac_groupA(CompanyName,CustId);
		}
		
		if(field.equals("ac_group")&&type.equals("DESC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.ac_groupD(CompanyName,CustId);
		}
		
		if(field.equals("open_balance")&&type.equals("ASC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.open_balanceA(CompanyName,CustId);
			 System.out.println("open balance asc");
		}
		
		if(field.equals("open_balance")&&type.equals("DESC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.open_balanceD(CompanyName,CustId);
			 System.out.println("open balance desc");
		}
		
		if(field.equals("mobile")&&type.equals("ASC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.mobileA(CompanyName,CustId);
		}
		
		if(field.equals("mobile")&&type.equals("DESC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.mobileD(CompanyName,CustId);
		}
		
		if(field.equals("email")&&type.equals("ASC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.emailA(CompanyName,CustId);
		}
		
		if(field.equals("email")&&type.equals("DESC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.emailD(CompanyName,CustId);
		}
		
		if(field.equals("bank")&&type.equals("ASC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.bankA(CompanyName,CustId);
		}
		
		if(field.equals("bank")&&type.equals("DESC"))
		{
			 li=(List<Account_ledger_v3>) ledgerServiceRepo.bankD(CompanyName,CustId);
		}
    ...
    public List<Account_ledger_v3> ledger_bn_dates(String start, String end,String CompanyName,String CustId)  {
	
	System.out.println("start"+start+"end"+end);
	List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.selectDataBnDate(start,end,CompanyName,CustId);
...
//		transactionServiceRepo.transactionDelete(id);
    ...
    public List<Account_ledger_v3> bank_names(String CompanyName,String CustId)  {
		
		
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.selectBank(CompanyName,CustId);
    ...
    public List<Account_transactions_v3> account_statementDatas(int ledgerId)  {
 		...
 		{  
 		 ...
 		 System.out.println("line 313 balance_type  "+balance_type);
...
li1.get(0).setCreatedTime(Double.toString( balance));
    ...
    public List<Account_ledger_v3> profit_losss(String title, String start, String end, String CompanyName, String CustId)  {
    	
     	
    	double directIncomeAmount=(double) 0;
    	int ledgerId;
    	double ledgerTotalAmount=(double) 0;
    	double dirIncome=(double) 0;
    	...
    	List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.profit_loss_idSearchByTitle(title);
    	 		...
    	 		for (int i = 0; i < li.size(); i++)   
    	 			...
    	 			String s=String.valueOf(ledgerId);
    	 			
    	 			List<Account_transactions_v3> li1=(List<Account_transactions_v3>) transactionServiceRepo.profit_loss_bn_date(ledgerId,start,end,CompanyName,CustId);
    	 			System.out.println("ledgerId "+ledgerId+" start "+start+" end "+end+" CompanyName "+CompanyName);
        	 		...
        	 		for (int j = 0; j < li1.size(); j++)   
        	 			{ 
        	 		    System.out.println("directIncomeAmount  "+directIncomeAmount+"li1.get(j).getCrdt_ac() "+li1.get(j).getCrdt_ac()+"s = "+s+"Float.parseFloat(li1.get(j).getAmount()) "+Float.parseFloat(li1.get(j).getAmount()));
	    	 			//System.out.println("li1[j] "+li1[j]);
						if(title.equals("7")||title.equals("9"))
						{
						if(li1.get(j).getCrdt_ac().equals(s))
						{
							System.out.println("Credit Account");
							dirIncome = 0.00 - Float.parseFloat(li1.get(j).getAmount());
						}
						if(li1.get(j).getDbt_ac().equals(s))
						{
							System.out.println("Debit Account");
							dirIncome = 0.00 + Float.parseFloat(li1.get(j).getAmount());
						}
						}
						if(title.equals("6")||title.equals("8"))
						{
						if(li1.get(j).getDbt_ac().equals(s))
						{
							System.out.println("Credit Account");
						...
						if(li1.get(j).getCrdt_ac().equals(s))
						{
    	 				...
    	 				System.out.println("dirIncome "+dirIncome);		
    	 		    directIncomeAmount=(double) (directIncomeAmount+dirIncome);
    	 		         	 		System.out.println("directIncomeAmount  "+directIncomeAmount);

    	 		    ledgerTotalAmount=(double) (ledgerTotalAmount+dirIncome);
    	 		        	 		System.out.println("ledgerTotalAmount  "+ledgerTotalAmount);
    ...
    public List<Account_ledger_v3> profit_loss_bn_dates(String title, String start, String end,String CompanyName,String CustId)  {
    	...
    	List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.profit_loss_idSearchByTitle(title);
    	 		...
    	 		System.out.println("title "+title+" li.size() "+ li.size());
    	 		...
    	 		for (int i = 0; i < li.size(); i++)   
					...
					System.out.println("ledgerId "+ledgerId);
    	 			...
    	 			System.out.println("ledgerId "+ledgerId+" start "+start+" end "+end+" CompanyName "+CompanyName+" size "+ li1.size());
    	 			System.out.println("li1 "+li1);
    ...
    public List<Account_transactions_v3> account_statementDataBnDates(int ledgerId,String start, String end,String CompanyName,String CustId)  {
 		System.out.println("ledgerId "+ledgerId);
 		...
 		{  
 		 ...
 		 System.out.println("line 716 balance_type  "+balance_type);
...
//    }
   ...
   public List<Account_ledger_v3> balanceSheetDataBnDates(String title,String Start, String end,String CompanyName,String CustId)  {
		System.out.println("title "+title+" Start Date "+Start+" end date "+end);
		...
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.profit_loss(title,CompanyName,CustId);	
		...
		{ 
			...
			if(!li.get(i).getAmount().equals("0") && !li.get(i).getAmount().equals(""))
			{
				...
				List<Account_transactions_v3> li1=(List<Account_transactions_v3>) transactionServiceRepo.selectBalanceSheetDataBnDates(id,start,end,CompanyName,CustId);
				System.out.println("id "+id+" start "+start+" end "+end+" CompanyName "+CompanyName);
				System.out.println("li1.size() "+ li1.size());
				
				
				for (int j = 0; j < li1.size(); j++)   
		 		{  		 			
					System.out.println("Before CALCULATION debit_total "+ debit_total+" credit_total "+ credit_total);
					System.out.println("dbtAc "+ li1.get(j).getDbt_ac()+" CrdtAc "+ li1.get(j).getCrdt_ac()+" li1.get(j).getAmount() "+li1.get(j).getAmount());
				    ...
				    System.out.println("After  CALCULATION debit_total "+ debit_total+" credit_total "+ credit_total);		 	 	        
			    }				
				
				if(title.equals("10")||title.equals("4")||title.equals("3"))
				{
					balance=(double) (credit_total-debit_total);
				}               
               if(title.equals("2")||title.equals("1"))
			   {
					balance=(double) (debit_total-credit_total);
			   }

				System.out.println("debit_total "+ debit_total+" credit_total "+ credit_total);
				System.out.println("balance "+ balance);
...
//    }
    ...
    public List<Account_ledger_v3> balanceSheetProfitLossDataBnDates(String title, String start  , String end,String CompanyName,String CustId)  {
 		System.out.println("title "+title+" start "+start+" end "+end);
 		
 		double directIncomeAmount=(double) 0;
    	
 		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.balanceSheetProfitLossDataBnDate(title,start ,end,CompanyName,CustId);
    ...
    public List<Account_ledger_v3> trial_balance(String acType,String CompanyName,String CustId)  {
...
List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.trial_balances(acType,CompanyName,CustId);
...
// 		System.out.println("directIncomeAmount  "+directIncomeAmount);
    ...
    public String trial_balance_total(String CompanyName,String CustId)  {
    	
    	Double totalDbt= transactionServiceRepo.selectTotalDbtAmnt(CompanyName,CustId);
			
			System.out.println("totalDbt "+ totalDbt);
		
			Double totalCrd= transactionServiceRepo.selectTotalCrdAmnt(CompanyName,CustId);
    ...
    public List<Account_ledger_v3> trial_balanceBnDate(String acType,String start,String end,String CompanyName,String CustId)  {
    ...
    List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.trial_balances(acType,CompanyName,CustId);
     		...
     		for (int i = 0; i < li.size(); i++)   
     			...
     			Double dbtAmnt=transactionServiceRepo.selectTrialBalanceDbtBnDates(id,start,end,CompanyName,CustId);
     			...
     			Double crdtAmnt=transactionServiceRepo.selectTrialBalanceCrdtBnDates(id,start,end,CompanyName,CustId);
...
//     		System.out.println("directIncomeAmount  "+directIncomeAmount);
   ...
   public String trial_balance_totalBnDate(String start,String end,String CompanyName,String CustId)  {
    	
    	Double totalDbt= transactionServiceRepo.selectTotalDbtAmntBnDates(start,end,CompanyName,CustId);
			
			System.out.println("totalDbt "+ totalDbt);
		
			Double totalCrd= transactionServiceRepo.selectTotalCrdAmntBnDates(start,end,CompanyName,CustId);
   ...
   public List<Account_ledger_v3> bankDatas(String CompanyName,String CustId)  {
   	
   
   	
List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.bankData(CompanyName,CustId);
   ...
   public List<Account_ledger_v3> selectCustomers(String CompanyName,String CustId)  {
		
		
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.selectCustomer(CompanyName,CustId);
		
		return li;
    }
	

   public List<Account_ledger_v3> selectServices(String CompanyName,String CustId)  {
		
		
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.selectService(CompanyName,CustId);
	...
	public List<Account_ledger_v3> ac_dashboardCashData(String CompanyName)  {
		
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.ac_dashboardCashDatas(CompanyName);
    ...
    public List<Account_ledger_v3> ac_dashboardBankData(String CompanyName)  {
		
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.ac_dashboardBankDatas(CompanyName);
    ...
    public List<Account_ledger_v3> index_customer_vendorApi(String grp,String CompanyName,String CustId)  {
		
		List<Account_ledger_v3> li=(List<Account_ledger_v3>) ledgerServiceRepo.index_customer_vendorApis(grp,CompanyName,CustId);
